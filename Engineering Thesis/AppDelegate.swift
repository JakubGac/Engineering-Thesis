...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ app: UIApplication, open url: URL, options: [UIApplicationOpenURLOptionsKey : Any] = [:]) -> Bool {
        if url.pathExtension == "ex" {
            ...
            return true
        } else if url.pathExtension == "tean" {
            DaoManager().importAnswers(from: url)
            if let navigationController = window?.rootViewController as? UINavigationController {
                if let mainMenuViewController = navigationController.viewControllers.first as? MainMenuViewController {
                    mainMenuViewController.performSegue(withIdentifier: "Exam Added", sender: nil)
                }
    ...
    func applicationWillResignActive(_ application: UIApplication) {
        ...
        // ktoś tu próbował ściągać, kończymy test i usuwamy wszystkie odpowiedzi
        if StudentTestDao().checkIfStudentTestLast() {
            ...
            if let navigationController = window?.rootViewController as? UINavigationController {
                ...
                if let mainMenuViewController = navigationController.viewControllers.first as? MainMenuViewController {
                    mainMenuViewController.performSegue(withIdentifier: "End Test Segue", sender: nil)
                }
            }
        }
    }
